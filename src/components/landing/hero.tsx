      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col items-center relative z-10">
        ...
        >
          ...
          <p className="text-zinc-500 text-lg md:text-xl max-w-xl mx-auto mb-16 leading-relaxed font-medium tracking-tight">
            Foque no que importa. O visualizador de cifras definitivo para
            músicos de alta performance.
          </p>
        </motion.div>
          ...
          <div className="relative group p-[1px] rounded-full overflow-hidden">
            ...
            <div
              className={cn(
                ...
                isFocused
                  ? "bg-black shadow-[0_0_80px_-15px_rgba(234,179,8,0.25)]"
                  : "",
              )}
            >
              <div className="pl-7 pr-4">
                <Search
                  className={cn(
                    ...
                    isFocused ? "text-yellow-500 scale-110" : "text-zinc-800",
                  )}
                />
              </div>
        ...
        >
          <div className="flex items-center gap-5 mb-12">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-zinc-400 text-[11px] font-black uppercase tracking-[0.5em]">
                Tendências
              </h3>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {isLoadingTrending
              ? Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-24 rounded-3xl bg-white/[0.01] border border-white/[0.03] animate-pulse"
                  />
                ))
              : trendingSongs
                  .slice(0, 6)
                  .map((song, i) => (
                    <TrendingCard
                      ...
                      onClick={() =>
                        router.push(
                          `/song?id=${typeof btoa !== "undefined" ? btoa(encodeURIComponent(song.url)) : ""}`,
                        )
                      }
                    />
                  ))}
